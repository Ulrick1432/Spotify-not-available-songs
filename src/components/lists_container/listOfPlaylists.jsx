  ...
  const handleClickPlaylists = async (index, playlist_id, totalTracksInPlaylist) => {
    ...
    if (totalTracksInPlaylist < 101) {
      try {
        ...
        setTooManyTracksInPlaylist(false);
        setLoadingListOfNotAvailableSongs(false);
      }
    } else {
      console.log('Playlist has more than 100 trakcs/songs. Spotify API cant handle that because of bug - there is no fix...');
      setTooManyTracksInPlaylist(true);
      setLoadingListOfNotAvailableSongs(false);
    }
